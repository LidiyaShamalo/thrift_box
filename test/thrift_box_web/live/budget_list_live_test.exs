defmodule ThriftBoxWeb.BudgetListLiveTest do
  ...
  describe "Create budget modal" do
    test "modal in presented", %{conn: conn, user: user} do
      conn = log_in_user(conn, user)
    ...
    test "validation errors are presented when form is changed with invalid input", %{
      ...
      form =
        form(lv, "#create-budget-modal form", %{ #element(lv, "#create-budget-modal form")
            "budget" => %{"name" => ""}
      })
  ...
  test "validation errors are presentes when form is submitted with invalid input", %{
    conn: conn,
    user: user
  } do
    conn = log_in_user(conn, user)
